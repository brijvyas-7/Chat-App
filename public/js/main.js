...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function addVideoElement(type, userId, stream, isLocal = false) {
    ...
    v.autoplay = true;
    v.playsInline = true;
    v.muted = isLocal; // Always mute local video
    ...
    // Modified play handling with error recovery
  ...
  async function startCall(t) {
    ...
    // First get permission and verify access
    try {
    ...
    try {
      ...
      // Ensure the call UI is shown before any media plays
      showCallUI(t);

      // Start the call after UI is visible
